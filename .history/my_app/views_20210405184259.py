...
def update_blog(request, pk):
    p_k = int(pk)
    model = BlogModel.objects.get(id=p_k)
    if request.method == 'POST':
        title = request.POST.get('title')
    ...
    return render(request, 'update_blog.html', {'pk':p_k})
def blog_detail(request, id):
    blog = BlogModel.objects.get(id=id)
    return render(request, 'blog_detail.html',{'blog':blog,'model.category_id':model.category_id})
